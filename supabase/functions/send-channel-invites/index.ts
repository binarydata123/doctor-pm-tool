...
async function sendInviteEmail({
  ...
  const subject = `Join #${channelName} on DPM-Tool`;
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #0f172a;">
      <p><strong>${escapeHtml(inviterName)}</strong> invited you to join <strong>#${escapeHtml(channelName)}</strong> on DPM-Tool.</p>
      <p>
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "DPM-Tool <noreply@pmtool.ai-developer.site>",
      to: recipientEmail,
      subject,
      html,
    }),
